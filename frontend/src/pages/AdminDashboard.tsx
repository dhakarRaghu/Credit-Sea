...
const AdminDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (!user || !["ADMIN", "VERIFIER"].includes(user.role)) {
        setError("Access denied: Admin or Verifier role required");
        setLoading(false);
        return;
      }
      try {
  ...
  const openLoanPopup = (loan: Loan) => setSelectedLoan(loan);
  const closeLoanPopup = () => setSelectedLoan(null);

  if (loading) {
    return <div className="min-h-screen bg-gray-200 flex items-center justify-center">Loading...</div>;
  }
      ...
      <header className="bg-white shadow-sm p-4">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center space-x-4">
            ...
            <button className="text-sm text-green-700 hover:underline">Admin</button>
          </div>
          <div className="flex items-center space-x-4">
        ...
        <main className="flex-1 p-6">
          ...
          <div className="grid grid-cols-4 gap-4 mb-6">
            ...
            <div className="bg-green-800 text-white p-4 shadow rounded-lg text-center">
              ...
              <p className="text-2xl font-bold">{stats?.otherAccounts || 0}</p>
            </div>
            <div className="bg-white p-4 shadow rounded-lg text-center">
          ...
          <div className="bg-white shadow rounded-lg p-4">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold text-gray-700">Recent Loans</h3>
